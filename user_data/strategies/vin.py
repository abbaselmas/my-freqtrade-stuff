...
class ViN(IStrategy):
    ...
    def populate_indicators_1h(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        # df['hlc3_low_w'] = df['hlc3_adj'].rolling(window=7*24, min_periods=1).min()
        return df
...
class ViNBuyPct(ViN):
    ...
    def populate_buy_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[:, 'buy'] = df['buy_tag'].ne('') & ~df['buy_tag'].str.contains('21 28|22 28|24 28|25 28')
...
class ViNBuyPctLc2(ViN):
    ...
    def populate_buy_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        buy_pct = df['buy_tag_pct'].ne('') & ~df['buy_tag_pct'].str.contains('24 28')
        buy_lc2 = df['buy_tag_lc2'].eq('15 16 17 18 19 20 ')
        df.loc[:, 'buy'] = buy_pct | buy_lc2
        df.loc[buy_pct, 'buy_tag'] = 'pct ' + df['buy_tag_pct']
        df.loc[buy_lc2, 'buy_tag'] += 'lc2 ' + df['buy_tag_lc2']
...
class ViNBuyLow(ViN):
    def populate_buy_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        buy_conditions = [
            df['candle_count_1h'].ge(self.min_day_listed * 24),
            # df['volume'].ge(df[f"volume_mean_{i}"].shift(1)),
            (df['hlc3_adj_1h'].shift(1) / df['hlc3_low_w_1h'].shift(24)).between(0.97, 0.98)
            # (df['hlc3_adj_1h'] / df['hlc3_low_1h']).ge(1),
            # (df['hc2_adj'] / df['close']).le(1.01)
            # (df['lc2_adj_1h'] / df['close_1h']).between(0.945, 0.995)
        ]
        buy = reduce(lambda x, y: x & y, buy_conditions)
        df.loc[buy, 'buy_tag'] = 'low'
    ...
    def custom_sell(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        if rise > 1.08:
            if fall > pow(rise, 0.33) - trade_len / 1000:
                ...
                return f"rise {u}"
# or red candle with long upper tail
        elif rise < 1.04:
...
class ViNLowRiseFall(ViNBuyLow, ViNSellRiseFall):
    pass
