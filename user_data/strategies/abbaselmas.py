...
class abbas6(IStrategy):
    ...
    buy_trend_optimize = True
    rsi_fastX = IntParameter(25, 45, default=buy_params['rsi_fastX'], space='buy', optimize=buy_trend_optimize)
    rsiX = IntParameter(20, 30, default=buy_params['rsiX'], space='buy', optimize=buy_trend_optimize)
    profit = DecimalParameter(0.9, 1.1, default=protection_params['profit'], space='buy', decimals=3, optimize=buy_trend_optimize)

    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:

        dataframe.loc[
            (
                (dataframe['rsi_fast'] < buy_params['rsi_fastX']) &
        ...
        dont_buy_conditions.append(
            (
                (dataframe['close_1h'].rolling(24).max() < (dataframe['close'] * buy_params['profit'] )) # don't buy if there isn't 3% profit to be made
            )
        )
