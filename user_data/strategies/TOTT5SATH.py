...
class TOTT5SATH(IStrategy):
    class HyperOpt:
        ...
        def generate_estimator(dimensions: List['Dimension'], **kwargs):
            ...
            # return "ET"
            return ExtraTreesRegressor(n_estimators=5, n_jobs=-1)
    ...
    s_second_ott_smoothing = IntParameter(2, 4, default=2, space='buy', optimize=s_optimize_second, load=True)
    ...
    s_third_ott_smoothing = IntParameter(2, 4, default=2, space='buy', optimize=s_optimize_third, load=True)
    ...
    s_fourth_ott_smoothing = IntParameter(2, 4, default=2, space='sell', optimize=s_optimize_fourth, load=True)
    ...
    s_fifth_ott_smoothing = IntParameter(2, 4, default=2, space='sell', optimize=s_optimize_fifth, load=True)
    ...
    s_hhv1_length = IntParameter(2, 6, default=4, space='buy', optimize=optimize_patches, load=True)
    s_hhv2_length = IntParameter(2, 6, default=4, space='buy', optimize=optimize_patches, load=True)

    s_hott1_length = DecimalParameter(0.4, 0.6, default=0.4, decimals=1,
                                    space="buy", optimize=optimize_patches, load=True)

    optimize_patches2 = False
    s_lott1_length = DecimalParameter(0.4, 0.6, default=0.4, decimals=1,
                                    space="buy", optimize=optimize_patches2, load=True)
    s_llv1_length = IntParameter(2, 6, default=4, space='buy',
                               optimize=optimize_patches2, load=True)
    s_llv2_length = IntParameter(2, 6, default=4, space='buy',
                               optimize=optimize_patches2, load=True)
    s_llv3_length = IntParameter(2, 6, default=4, space='buy',
                               optimize=optimize_patches2, load=True)
    #1250
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
       ...
       #2.Bolge
        ...
        # 5.Bolge
        dataframe['FIFTH_UP'], _ = myind.ott_smt_signal(heikinashi, window=int(self.s_fourth_vidya_length.value) * 10, percent=self.s_fourth_ott_length.value * 2.0, smoothing=( self.s_fourth_ott_smoothing.value * 0.0002), field='close', matype='var')
        dataframe['FIFTH_STSK'] = myind.StochVar_Signal(heikinashi, fastk_period=self.s_fourth_stsk_length.value*50, fastd_period=self.s_fourth_stsd_length.value*50, smoothing=self.s_fourth_perc_length.value)
            
        # PATCHES
        dataframe['HHV1'] = myind.hhv_signal(dataframe, period=self.s_hhv1_length.value * 10)
        dataframe['HHV2'] = myind.hhv_signal(dataframe, period=self.s_hhv2_length.value * 10)

        dataframe['HOTT1'] = myind.hott_signal(dataframe, period=self.s_hhv1_length.value*5, shft=0, perc=self.s_hott1_length.value)
        dataframe['HOTT2'] = myind.hott_signal(dataframe, period=self.s_hhv2_length.value*5, shft=0, perc=self.s_hott1_length.value)

        # dataframe['LLV1'] = myind.llv_signal(dataframe, period=self.llv1_length.value * 10)
        dataframe['LLV2'] = myind.llv_signal(dataframe, period=self.s_llv2_length.value * 10)
        dataframe['LLV3'] = myind.llv_signal(dataframe, period=self.s_llv3_length.value * 10)


        # dataframe['LOTT1'] = myind.lott_signal(dataframe, period=self.llv1_length.value*5, shft=0, perc=self.hott1_length.value)
        dataframe['LOTT2'] = myind.lott_signal(dataframe, period=self.s_llv2_length.value*5, shft=0, perc=self.s_lott1_length.value)
        dataframe['LOTT3'] = myind.lott_signal(dataframe, period=self.s_llv3_length.value*5, shft=0, perc=self.s_lott1_length.value)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 3.Bolge
        _, dataframe['THIRD_DOWN'] = myind.ott_smt_signal(heikinashi, window=int(self.s_third_vidya_length.value) * 10, percent=self.s_third_ott_length.value*2.0, smoothing=(self.s_third_ott_smoothing.value * 0.0002), field='close', matype='var')
        dataframe['THIRD_STSK'] = myind.StochVar_Signal(heikinashi, fastk_period=self.s_third_stsk_length.value*50, fastd_period=self.s_third_stsd_length.value*50, smoothing=self.s_third_perc_length.value)
