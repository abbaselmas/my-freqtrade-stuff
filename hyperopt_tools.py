...
from freqtrade.optimize.optimize_reports import generate_wl_ratio
...
class HyperoptTools:
    ...
    def format_results_explanation_string(results_metrics: Dict, stake_currency: str) -> str:
        ...
        return (f"{results_metrics['total_trades']:6d} trades. "
                f"{results_metrics['wins']} wins. "
                f"{results_metrics['draws']} draws. "
                f"{results_metrics['losses']} losses. "
    ...
    def prepare_trials_columns(trials: pd.DataFrame, has_drawdown: bool) -> pd.DataFrame:
        ...
        if 'results_metrics.wl_ratio' not in trials.columns:
            # Ensure compatibility with older versions of hyperopt results
            trials['results_metrics.wl_ratio'] = 'N/A'
        ...
        trials['results_metrics.wl_ratio'] = trials.apply(
            lambda x: generate_wl_ratio(
                            x['results_metrics.wins'], x['results_metrics.draws'],
                            x['results_metrics.losses']
                      ), axis=1)

        trials = trials[['Best', 'current_epoch', 'results_metrics.total_trades',
                         'results_metrics.wins', 'results_metrics.draws', 'results_metrics.losses',
                         'results_metrics.wl_ratio',
